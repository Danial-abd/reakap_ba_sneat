    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('up.td', $teamdetail->id) }}" method="POST" class="form-item">
                        @csrf
                        <div class="row mb-3">
                            ...
                            <select name="id_team" id="id_team" class="form-control">
                                <option value="" disabled selected>--Pilih Nama Tim--</option>
                                    @foreach($teamlist as $tl)
                                        <option value="{{$tl->id}}" {{$teamdetail->id_team == $tl->id ? 'selected' : '' }}>{{$tl->list_tim}}</option>    
                                    @endforeach
                            </select>
                        </div>
                        </div>
                            ...
                            <select name="id_karyawan" id="id_team" class="form-control">
                                <option value="" disabled selected>--Pilih Nama Karyawan--</option>
                                    @foreach($karyawan as $k)
                                        <option value="{{$k->id}}" {{$teamdetail->id_karyawan == $k->id ? 'selected' : '' }}>{{$k->nama}}</option>    
                                    @endforeach
                            </select>
                        </div>
                        </div>
                            ...
                            <select name="id_jobdesk" id="id_jobdesk" class="form-control">
                                <option value="" disabled selected>--Pilih Nama Pekerjaan--</option>
                                    @foreach($jobdesk as $jd)
                                        <option value="{{$jd->id}}" {{$teamdetail->id_jobdesk == $jd->id ? 'selected' : '' }}>{{$jd->jobdesk}} {{ $jd->detail_kerja}}</option>    
                                    @endforeach
                            </select>
                        </div>
                        </div>
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <textarea name="ket" id="alamat" class="form-control" cols="2" rows="3" placeholder="Masukkan Alamat"
                                    style="resize : none">{{ $teamdetail->ket }}</textarea>
                            </div>
                        </div>
